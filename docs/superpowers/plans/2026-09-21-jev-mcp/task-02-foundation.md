...
- Create: `tests/test_errors.py`, `tests/test_paths.py`, `tests/test_redact.py`, `tests/test_responses.py`, `tests/test_logger.py`
...
`configure()` binds its handler to the current `sys.stderr`, so calling it inside each
test lets `capsys` capture the output. Each test sets the format it needs because the
module keeps the format and redaction flags in module-level state.
